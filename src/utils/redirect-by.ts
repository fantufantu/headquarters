...
export const redirectBy = (
  searchParams?:
    | Voidable<Record<string, string>>
    | ((options: Options) => Voidable<Record<string, string>>),
) => {
  const _redirectTo = new URL(globalThis.window.location.href).searchParams.get(
    AuthenticationToken.Redirect,
  );
